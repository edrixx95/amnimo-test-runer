...
export const getAggregatedReport = async (sessionId: string, session: Session) => {
  ...
  if (fs.existsSync(reportsDir)) {
    ...
    for (const dateDir of dateDirs) {
      ...
      const runDirs = fs.readdirSync(level1Path, { withFileTypes: true }).filter(d => {
  ...
  for (let i = 0; i < runDirsList.length; i++) {
    ...
    const htmlReportUrl = hasHtml ? `/api/reports/serve/${run.relativePath}/html-report/index.html` : null;
            ...
            for (const page of cat.pages) {
              if (Array.isArray(page.tests)) {
                for (const test of page.tests) {
                  ...
                  let status = 'Failed';
                  if (rawResult.toLowerCase().includes('pass')) status = 'Passed';
                  else if (rawResult.toLowerCase().includes('skip')) status = 'Skipped';

                  const pwTestId = test['playwright-test-id'] || '';
                  const specificHtmlReportUrl = hasHtml && pwTestId ? `${htmlReportUrl}#?testId=${pwTestId}` : htmlReportUrl;
  ...
  for (const test of testMap.values()) {
    const pages = aggregatedCategories.get(test.category) ?? new Map<string, any[]>();
    ...
    tests.push({
      ...
      'latest_result': test.history.length > 0 ? test.history[test.history.length - 1].result : '-',
      'latest_status': test.history.length > 0 ? test.history[test.history.length - 1].status : 'Failed',
    });
    ...
    totalCount++;
    const latestStatus = test.history.length > 0 ? test.history[test.history.length - 1].status : 'Failed';
