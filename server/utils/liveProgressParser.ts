...
export const getLiveProgress = async (sessionId: string): Promise<SessionMeta | null> => {
  ...
  try {
    ...
    type InnerTest = { name: string; status: 'running' | 'PASSED' | 'FAILED' | 'SKIPPED' };
    type Spec = { path: string; status: 'waiting' | 'running' | 'passed' | 'failed'; innerTests: InnerTest[] };
    ...
    for (const line of lines) {
      ...
      const execMatch = cleanLine.match(/^\[(\d+)\/(\d+)\] Executing:\s+(.*?\.spec\.ts)(?::\d+)?$/);
      if (execMatch) {
      ...
      const testEndMatch = cleanLine.match(/^\[.*?\] TEST END status=(.*?)\s+duration=(.*)$/);
      if (testEndMatch) {
        const activeSpec = queuedSpecs.find(s => s.status === 'running');
        if (activeSpec && activeSpec.innerTests.length > 0) {
          const runningTest = activeSpec.innerTests.slice().reverse().find(t => t.status === 'running');
          if (runningTest) {
      ...
      const runFinishedMatch = cleanLine.match(/^\[.*?\] Run finished: status=(.*?)\s+/);
      if (runFinishedMatch) {
        const activeSpec = queuedSpecs.find(s => s.status === 'running');
        if (activeSpec) {
          activeSpec.status = runFinishedMatch[1].trim() === 'PASSED' ? 'passed' : 'failed';
        }
      }
    }
    ...
    const completedSpecs = queuedSpecs.filter(s => s.status === 'passed' || s.status === 'failed').length;
