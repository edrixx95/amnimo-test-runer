...
export default defineEventHandler(async (event) => {
  ...
  if (!targetPath) {
    throw createError({ statusCode: 400, statusMessage: 'targetPath is required' });
  }
  ...
  try {
    ...
    for (const suite of suites) {
      ...
      if (relPath.includes(typePrefix)) {
        relPath = relPath.substring(relPath.indexOf(typePrefix) + typePrefix.length);
      } else if (relPath.includes(typePrefix2)) {
        relPath = relPath.substring(relPath.indexOf(typePrefix2) + typePrefix2.length);
      }
    ...
    throw createError({ statusCode: 500, statusMessage: error.message || String(error) });
  }
});
