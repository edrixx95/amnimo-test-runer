...
export default defineEventHandler(async (event) => {
  ...
  if (mode === 'single') {
    ...
    const orderFile = (tests && typeof tests === 'string') ? tests : (testType === 'release' ? 'AG10.json' : 'custom.json');
    const orderFilePath = path.join(cwd, 'test-order', testType || 'release', orderFile);
    try {
  ...
  const resolvedFiles = items.map(item => {
    if (item.endsWith('.spec.ts')) {
      return item.includes('playwright/tests/release') ? item : `playwright/tests/release/${item}`;
    }
