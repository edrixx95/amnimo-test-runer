...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    console.log(`[Nx Witness Register] Attempting default login to ${loginUrl}`);
    ...
    if (authResponse.status === 401 || authResponse.status === 403) {
      return { 
        success: true, 
        message: 'Device already registered or password changed. Skipping factory setup.',
    ...
    if (authResponse.status !== 200 && authResponse.status !== 201) {
      throw createError({
        ...
        statusMessage: 'Failed to authenticate with Nx Witness server for registration',
      });
    }
    ...
    throw createError({
      ...
      statusMessage: error.message || error.statusMessage || 'Failed to communicate with Nx Witness server',
    });
  }
});
