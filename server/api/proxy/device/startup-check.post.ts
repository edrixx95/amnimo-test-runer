  ...
  try {
    ...
    if (needsSetup) {
      ...
      return {
        ...
        message: 'Device was uninitialized. Admin password has been set automatically.',
        detail: postResponse
      };
    } else {
      return {
    ...
    if (error.statusCode === 401 || error.statusCode === 404 || error.statusCode === 403) {
       return {
         initialized: true,
         actionTaken: 'none',
    ...
    throw createError({
      ...
      statusMessage: error.statusMessage || 'Failed to check device startup state',
    });
  }
});
