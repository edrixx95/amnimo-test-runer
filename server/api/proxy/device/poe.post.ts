  ...
  try {
    ...
    if (username && password) {
      try {
        ...
        if (err.status === 401) {
          throw createError({ statusCode: 401, statusMessage: 'Login failed: Invalid credentials' });
        }
        if (err.status !== 404) {
          throw err;
        }
    ...
    throw createError({ statusCode: 500, statusMessage: err.message || 'Failed to communicate with device' });
  } finally {
