...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (err.status === 401) {
      throw createError({ statusCode: 401, statusMessage: 'Unauthorized to access Partner GW. Please check credentials.' });
    }
    throw createError({ statusCode: 500, statusMessage: err.message || 'Failed to communicate with Partner GW API' });
  } finally {
