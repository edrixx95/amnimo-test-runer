...
export default defineEventHandler(async (event) => {
  ...
  const hasPlaywrightConfig = fs.existsSync(path.join(p, 'playwright.config.ts'));
  const hasPackageJson = fs.existsSync(path.join(p, 'package.json'));

  if (!hasPlaywrightConfig && !hasPackageJson) {
    return { valid: false, message: 'Missing playwright.config.ts or package.json. This might not be the correct amnimo-e2e folder.' };
  }
