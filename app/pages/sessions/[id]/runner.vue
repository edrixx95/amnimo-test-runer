...
const toggleTest = async () => {
  if (isTesting.value) {
    ...
    try {
      ...
      addE2ELog("\n\x1b[33m笞・・Sent kill signal to test process.\x1b[0m\n");
      if (session.value) session.value.status = "Failed";
    } catch (err) {
      addE2ELog("\n\x1b[31m笶・Failed to stop test process.\x1b[0m\n");
    }
  } else {
    // Start Execution
    isTesting.value = true;
    ...
    addE2ELog("\x1b[36m邃ｹ・・Initializing Test Runner...\x1b[0m\n");
    ...
    try {
      ...
      addE2ELog(
        `\n\x1b[33m笞・・Warning: Failed to parse test cases (${err.message}). Tests will appear as they run.\x1b[0m\n`,
      );
    } finally {
      isParsingTests.value = false;
    }
    ...
    try {
      ...
      isTesting.value = false;
      addE2ELog(
        `\n\x1b[31m笶・Failed to start test runner: ${err.message}\x1b[0m\n`,
      );
    }
  }
};
