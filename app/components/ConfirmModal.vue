<template>
  <Transition name="modal">
    <div v-if="modelValue" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-0">
      <div class="modal-backdrop fixed inset-0 bg-slate-900/60 backdrop-blur-sm transform-gpu will-change-opacity" @click="close"></div>
      ...
      <div class="modal-content relative bg-white rounded-2xl shadow-glass w-full max-w-md overflow-hidden border border-slate-200">
        <div class="px-6 py-6">
          <div class="flex items-start gap-4">
            <div class="shrink-0 flex items-center justify-center w-12 h-12 rounded-2xl" :class="iconBgClass">
              <Icon :name="iconName" class="w-7 h-7" :class="iconColorClass" />
            </div>
            <div class="pt-1 min-w-0 flex-1">
              <h3 class="text-xl font-bold text-slate-900">{{ title }}</h3>
              <p class="mt-2 text-sm font-medium text-slate-600 leading-relaxed">{{ message }}</p>
            </div>
          </div>
        </div>
        
        <div class="px-6 py-5 bg-slate-50 border-t border-slate-100 flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
          <button
            type="button"
            @click="close"
            :disabled="isLoading"
...
const confirmBtnClass = computed(() => {
  if (props.type === 'danger') return 'bg-red-600 hover:bg-red-700 dark:bg-red-600 dark:hover:bg-red-700';
  if (props.type === 'warning') return 'bg-amber-600 hover:bg-amber-700 dark:bg-amber-600 dark:hover:bg-amber-700';
