<template>
  <Transition name="modal">
    <div v-if="modelValue" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div class="modal-content relative bg-white rounded-2xl shadow-glass w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden">
        ...
        <div class="p-8 overflow-y-auto flex-1 bg-slate-50/50 min-h-[300px] custom-scrollbar">
          ...
          <div v-else class="space-y-4">
            <div v-for="report in reports" :key="report.id" class="bg-white border border-slate-200 hover:border-amnimo-300 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all duration-300 shadow-sm hover:shadow-soft group">
              <div>
                <h4 class="font-bold text-slate-900 flex items-center gap-2 text-lg group-hover:text-amnimo-700 transition-colors">
                  <Icon name="heroicons:folder-open" class="w-5 h-5 text-amnimo-400" />
                  {{ report.name }}
                </h4>
        ...
        <Transition name="modal">
          <div v-if="reportToDelete" class="absolute inset-0 z-[70] flex items-center justify-center p-4">
            <div class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm rounded-2xl" @click="reportToDelete = null"></div>
            <div class="relative bg-white border border-slate-200 shadow-xl rounded-2xl p-6 max-w-sm w-full mx-4 text-center transform transition-all">
...
const fetchReports = async () => {
  if (props.sessionId) {
    ...
    try {
      reports.value = await $fetch<ReportData[]>(`/api/sessions/${props.sessionId}/reports`);
    } catch (e) {
