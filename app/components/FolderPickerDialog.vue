<template>
  <Transition name="fade">
    <div v-if="modelValue" class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
      <div 
      ...
      >
        ...
        <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-full bg-amnimo-100 text-amnimo-600 flex items-center justify-center">
              <Icon name="heroicons:folder-open" class="w-4 h-4" />
            </div>
            <div>
              <h3 class="font-bold text-slate-800">Select Folder</h3>
              <p class="text-xs text-slate-500 font-mono mt-0.5 truncate max-w-sm" :title="currentPath">{{ currentPath }}</p>
            </div>
          </div>
        ...
        <div class="flex-1 h-[40vh] min-h-[300px] overflow-y-auto p-2 bg-white">
          <div v-if="isLoading" class="h-full flex items-center justify-center text-slate-400 gap-2">
            <AppSpinner size="sm" /> Loading...
          </div>
          <div v-else-if="errorMsg" class="h-full flex items-center justify-center text-rose-500 gap-2 px-6 text-center">
            <Icon name="heroicons:exclamation-circle" class="w-5 h-5 shrink-0" />
            <span class="text-sm font-medium">{{ errorMsg }}</span>
          </div>
          <div v-else-if="folders.length === 0" class="h-full flex items-center justify-center text-slate-400 text-sm">
          ...
          <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-1">
            <button
              ...
              :class="selectedFolder === folder ? 'bg-amnimo-50 text-amnimo-900 ring-1 ring-amnimo-200' : 'hover:bg-slate-50 text-slate-700'"
            >
              <Icon 
                :name="selectedFolder === folder ? 'heroicons:folder-open' : 'heroicons:folder'" 
                class="w-5 h-5 shrink-0 transition-colors"
                :class="selectedFolder === folder ? 'text-amnimo-500' : 'text-slate-400 group-hover:text-amber-400'" 
              />
        ...
        <div class="px-5 py-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between">
          ...
          <div class="flex gap-3">
            <button @click="close" class="px-4 py-2 rounded-xl text-slate-600 hover:bg-slate-200 font-bold text-sm transition-colors">
              Cancel
            </button>
            <button 
              @click="confirm"
...
async function loadDirectory(pathStr: string) {
  ...
  errorMsg.value = '';
  try {
    const res = await $fetch<{ currentPath: string, parentPath: string | null, folders: string[] }>('/api/utils/list-dir', {
      query: { path: pathStr }
    });
    ...
    errorMsg.value = err.data?.statusMessage || err.message || 'Cannot access this folder';
  } finally {
