<template>
  <Transition name="slide-up">
    <div v-if="isVisible" class="fixed bottom-6 right-6 z-[10000] bg-white rounded-2xl shadow-xl border border-slate-200 p-5 max-w-sm w-full flex flex-col gap-3">
      <div class="flex items-start gap-3">
        <div class="shrink-0 pt-0.5">
          <Icon name="heroicons:arrow-path-rounded-square" class="w-6 h-6 text-amnimo-500" :class="{ 'animate-spin': status === 'checking' || status === 'downloading' }" />
        </div>
        ...
        <div class="flex-1 flex flex-col gap-1 min-w-0">
          ...
          <div v-if="status === 'downloading'" class="mt-2 h-2 w-full bg-slate-100 rounded-full overflow-hidden">
            <div class="h-full bg-amnimo-500 transition-all duration-300" :style="{ width: `${progress}%` }"></div>
          </div>
        </div>
