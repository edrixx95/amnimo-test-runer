<template>
  <div class="relative min-w-[140px]" ref="dropdownRef">
    ...
    >
      ...
      <Icon name="heroicons:chevron-down" class="w-4 h-4 text-slate-400 transition-transform duration-200 shrink-0" :class="{ 'rotate-180': isOpen }" />
    </div>
    ...
    >
      <div v-if="isOpen" class="absolute z-50 w-full min-w-max mt-2 bg-white border border-slate-200 rounded-xl shadow-glass overflow-hidden right-0 origin-top-right">
        <div class="max-h-60 overflow-y-auto custom-scrollbar py-1.5">
          ...
          >
            ...
            <Icon v-if="modelValue === option.value" name="heroicons:check" class="w-4 h-4 text-amnimo-600 shrink-0" />
          </div>
        </div>
      </div>
    </Transition>
