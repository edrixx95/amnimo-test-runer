<template>
  <div
    ...
    :class="[
      ...
      (!modelValue && !errorMsg) ? 'border-slate-200 bg-white hover:border-slate-300' : ''
    ]"
  >
    <div class="flex justify-between items-start mb-5">
      <div class="flex items-center gap-4">
        <div 
          ...
          :class="[
            modelValue ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : '',
            (!modelValue && errorMsg) ? 'bg-amber-100 border-amber-200 text-amber-600' : '',
            (!modelValue && !errorMsg) ? 'bg-slate-50 border-slate-100 text-slate-500 group-hover:text-amnimo-500' : ''
          ]"
        >
    ...
    >
      ...
      <Transition name="fade">
        <div v-if="errorMsg" class="mt-4 pt-4 border-t border-slate-200">
          ...
          <div v-if="needsLogin" class="flex items-center gap-3 bg-white p-3 rounded-lg border border-rose-100 shadow-sm mt-3">
             <div class="relative flex-1">
               <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                 <Icon name="heroicons:user" class="w-4 h-4 text-slate-400" />
               </div>
               <input v-model="username" placeholder="Username" class="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-amnimo-500 focus:border-amnimo-500 bg-transparent" @keyup.enter="checkSim" />
             </div>
             <div class="relative flex-1">
               <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                 <Icon name="heroicons:key" class="w-4 h-4 text-slate-400" />
               </div>
               <input v-model="password" type="password" placeholder="Password" class="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-amnimo-500 focus:border-amnimo-500 bg-transparent" @keyup.enter="checkSim" />
             </div>
             <button @click="checkSim" class="px-4 py-2 bg-rose-600 text-white rounded-lg text-sm font-bold shadow-sm hover:bg-rose-700 transition-colors shrink-0 flex items-center gap-2">
      ...
      <!-- Success / Info Box -->
      <Transition name="fade">
        <div v-if="simInfo" class="mt-4 pt-4 border-t border-slate-200 space-y-3">
          <div class="flex justify-between items-center">
          ...
          <div class="flex justify-between items-center">
            ...
            <span class="font-bold text-slate-800 font-mono">{{ simInfo.imei }}</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-slate-500 font-medium">Firmware</span>
            <span class="font-medium text-slate-600 text-xs font-mono bg-white px-2 py-1 rounded border border-slate-200">{{ simInfo.fw_version }}</span>
          </div>
          <template v-if="simInfo.sim_iccid">
            <div class="flex justify-between items-center pt-2 border-t border-slate-100">
              <span class="text-slate-500 font-medium flex items-center gap-1.5"><Icon name="heroicons:credit-card" class="w-4 h-4" /> ICCID (Slot 0)</span>
              <span class="font-bold text-slate-800 font-mono text-xs">{{ simInfo.sim_iccid }}</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-slate-500 font-medium flex items-center gap-1.5"><Icon name="heroicons:finger-print" class="w-4 h-4" /> IMSI</span>
              <span class="font-bold text-slate-800 font-mono text-xs">{{ simInfo.sim_imsi }}</span>
            </div>
          </template>
        </div>
      </Transition>
...
async function checkSim() {
  if (!props.baseUrl) {
    errorMsg.value = 'Base URL is required to check SIM. Please set it in Step 1/2.';
    return;
  }
  ...
  try {
    ...
    if (module.sim && module.sim.length > 0) {
      ...
      if (sim0) {
        simInfo.value = { ...module, sim_iccid: sim0.iccid, sim_imsi: sim0.imsi };
        emit('update:modelValue', true);
      } else {
        simInfo.value = module;
        errorMsg.value = 'A SIM was detected, but it is NOT in Slot 0! Please move it to Slot 0.';
      }
    } else {
      // No SIM
      simInfo.value = module;
