...
const expectedDevices = computed<ExpectedDevice[]>(() => {
  ...
  if (["AG10", "AG20"].includes(b)) {
    return [
      ...
      {
        id: "sd",
        name: t("peripheralStorageCheck.sdCard"),
        regex: /^mmcblk1$/,
        icon: IconSdCard,
      },
    ];
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <span
                ...
                :class="
                  item.isConnected ? 'text-emerald-900' : 'text-slate-700'
                "
              >
                {{ item.name }}
              </span>
              <div
                class="flex items-center gap-2 mt-0.5 text-xs font-semibold uppercase tracking-wide"
                :class="
                  item.isConnected ? 'text-emerald-600/80' : 'text-amber-500'
                "
              >
